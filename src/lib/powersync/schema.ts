...
export async function getPowerSyncSchema() {
  ...
  try {
    ...
    _schema = new Schema({
      water_logs, caffeine_logs, meal_logs, pomodoro_sessions, sleep_configs,
      health_snapshots, health_profiles, sleep_logs, notes, projects, features,
      agents, skills, workflows, docs, delegations, sessions, plans, requests,
      persons, workspaces, teams, memberships, user_settings,
    })

    return _schema
  } catch (e) {
    console.warn('[PowerSync] Schema init failed:', e)
    return null
  }
}
