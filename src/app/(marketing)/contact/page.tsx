  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '72px 32px' }}>
      <div style={{ marginBottom: 48, textAlign: 'center' }}>
        <h1 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-0.04em', color: textPrimary, marginBottom: 12 }}>Contact us</h1>
        <p style={{ fontSize: 16, color: textMuted }}>Questions, enterprise inquiries, or just want to say hello.</p>
      </div>
      ...
      {sent ? (
        <div style={{ padding: '40px', borderRadius: 16, border: '1px solid rgba(34,197,94,0.2)', background: 'rgba(34,197,94,0.06)', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: textPrimary, marginBottom: 8 }}>Message sent!</h2>
          <p style={{ fontSize: 14, color: textMuted }}>We&apos;ll get back to you within 24 hours.</p>
        </div>
      ) : (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Your name" style={inputSt} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: labelColor }}>Subject</label>
              <input type="text" required value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} placeholder="How can we help?" style={inputSt} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: labelColor }}>Message</label>
              <textarea required value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} placeholder="Tell us more..." rows={6} style={{ ...inputSt, resize: 'vertical' as const }} />
            </div>
      ...
      <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[
          { icon: 'bxl-github', label: 'GitHub Discussions', href: 'https://github.com/orchestra-mcp/orchestra/discussions', color: isDark ? '#f8f8f8' : '#0f0f12' },
          { icon: 'bxl-discord-alt', label: 'Join our Discord', href: 'https://discord.gg/orchestra', color: '#5865f2' },
        ].map(link => (
          <a key={link.label} href={link.href} target="_blank" rel="noopener" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 10, border: `1px solid ${linkRowBorder}`, background: linkRowBg, textDecoration: 'none', color: textPrimary, fontSize: 14 }}>
            ...
            {link.label}
            <i className="bx bx-right-arrow-alt" style={{ marginLeft: 'auto', color: arrowColor }} />
          </a>
        ))}
      </div>
    </div>
