...
export default function SolutionsClient({ data }: { data: SolutionsData }) {
  ...
  const solutions = data.sections?.length
    ? data.sections.map(s => ({
        icon: s.icon ?? 'bx-star',
        title: s.title ?? '',
        subtitle: s.subtitle ?? '',
  ...
  const pageTitle = data.title || t('solutions.title')
  const pageSubtitle = data.subtitle || t('solutions.subtitle')
  const ctaTitle = data.cta_title || t('solutions.notSure')
  const ctaSubtitle = data.cta_subtitle || t('solutions.notSureSubtitle')
  ...
  return (
    <div className="mkt-page" style={{ maxWidth: 1100, margin: '0 auto', padding: '72px 32px' }}>
      <div ref={headerReveal.ref} style={{ marginBottom: 72, textAlign: 'center', ...revealStyle(headerReveal.isVisible) }}>
        <h1 style={{ fontSize: 'clamp(32px, 5vw, 52px)', fontWeight: 800, letterSpacing: '-0.04em', color: textPrimary, marginBottom: 14 }}>{pageTitle}</h1>
        <p style={{ fontSize: 17, color: textMuted, maxWidth: 520, margin: '0 auto' }}>{pageSubtitle}</p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
        {solutions.map((sol, idx) => (
          <SolutionCard key={sol.title} sol={sol} idx={idx} textPrimary={textPrimary} textBody={textBody} />
        ))}
      </div>

      <div ref={ctaReveal.ref} style={{ marginTop: 80, textAlign: 'center', ...revealStyle(ctaReveal.isVisible) }}>
        <h2 style={{ fontSize: 28, fontWeight: 700, color: textPrimary, marginBottom: 12 }}>{ctaTitle}</h2>
        <p style={{ fontSize: 16, color: textMuted, marginBottom: 28 }}>{ctaSubtitle}</p>
...
function SolutionCard({ sol, idx, textPrimary, textBody }: {
  sol: { icon: string; title: string; subtitle: string; color: string; useCases: { title: string; desc: string }[]; cta: string; href: string }
  idx: number; textPrimary: string; textBody: string
}) {
  const reveal = useScrollReveal({ threshold: 0.15 })
  return (
    <div ref={reveal.ref} style={{ padding: '40px', borderRadius: 20, border: `1px solid ${sol.color}20`, background: `${sol.color}05`, position: 'relative', overflow: 'hidden', ...revealStyle(reveal.isVisible, idx * 100) }}>
