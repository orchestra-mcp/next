    ...
    <div className="mkt-page" style={{ maxWidth: 1100, margin: '0 auto', padding: '72px 32px' }}>
      ...
      <div style={{ marginBottom: 56, textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 100, border: '1px solid rgba(0,229,255,0.25)', background: 'rgba(0,229,255,0.06)', marginBottom: 20, fontSize: 12, fontWeight: 600, color: '#00e5ff', letterSpacing: '0.04em', textTransform: 'uppercase' as const }}>
          {t('marketplace.officialPacks', { count: totalPacks })}
        </div>
        ...
        <p style={{ fontSize: 17, color: textMuted, maxWidth: 500, margin: '0 auto 32px' }}>{t('marketplace.subtitle')}</p>
      ...
      {/* Packs grid */}
      <div className="mkt-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
        {packs.map(pack => (
          <div key={pack.name} style={{ padding: '24px', borderRadius: 16, border: `1px solid ${cardBorder}`, background: cardBg, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              {[
                { label: t('marketplace.skills'), value: pack.skills },
                { label: t('marketplace.agents'), value: pack.agents },
                { label: t('marketplace.hooks'), value: pack.hooks },
              ].map(s => (
