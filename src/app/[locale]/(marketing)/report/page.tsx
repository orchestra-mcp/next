...
export default function ReportPage() {
  ...
  const issueTypes = [
    { key: 'Bug', label: t('report.bug') },
    { key: 'Feature Request', label: t('report.featureRequest') },
    { key: 'Security Vulnerability', label: t('report.securityVulnerability') },
    { key: 'Documentation', label: t('report.documentation') },
  ...
  return (
    <div className="mkt-page" style={{ maxWidth: 640, margin: '0 auto', padding: '72px 32px' }}>
      <div style={{ marginBottom: 48, textAlign: 'center' }}>
        <h1 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-0.04em', color: textPrimary, marginBottom: 12 }}>{t('report.title')}</h1>
        <p style={{ fontSize: 16, color: textMuted }}>{t('report.subtitle')}</p>
      </div>
      ...
      {sent ? (
        <div style={{ padding: '40px', borderRadius: 16, border: '1px solid rgba(34,197,94,0.2)', background: 'rgba(34,197,94,0.06)', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, color: textPrimary, marginBottom: 8 }}>{t('report.reportSubmitted')}</h2>
          <p style={{ fontSize: 14, color: textMuted }}>{t('report.reportSubmittedDesc')}</p>
        </div>
      ) : (
        <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 20, padding: '36px 32px', backdropFilter: 'blur(12px)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: labelColor }}>{t('report.issueType')}</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {issueTypes.map(it => (
                  <button
                    key={it.key}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, type: it.key }))}
                    style={{
                      padding: '7px 14px', borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit',
                      background: form.type === it.key ? 'rgba(0,229,255,0.12)' : optInactiveBg,
                      border: form.type === it.key ? '1px solid rgba(0,229,255,0.3)' : `1px solid ${optInactiveBorder}`,
                      color: form.type === it.key ? '#00e5ff' : optInactiveColor,
                    }}
                  >
                    {it.label}
                  </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: labelColor }}>{t('report.description')}</label>
              <textarea required value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder={t('report.descriptionPlaceholder')} rows={4} style={{ ...inputSt, resize: 'vertical' as const }} />
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: labelColor }}>{t('report.stepsToReproduce')} <span style={{ color: optionalLabelColor, fontWeight: 400 }}>({t('common.optional')})</span></label>
              <textarea value={form.steps} onChange={e => setForm(f => ({ ...f, steps: e.target.value }))} placeholder={t('report.stepsPlaceholder')} rows={4} style={{ ...inputSt, resize: 'vertical' as const }} />
            </div>
            ...
            {form.type === 'Security Vulnerability' && (
              <div style={{ padding: '12px 14px', borderRadius: 8, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', fontSize: 13, color: '#ef4444' }}>
                <i className="bx bx-shield-x" style={{ marginInlineEnd: 6 }} />
                {t('report.securityWarning')}
              </div>
            )}
      ...
      <div style={{ marginTop: 32, padding: '16px 20px', borderRadius: 10, border: `1px solid ${ghRowBorder}`, background: ghRowBg, display: 'flex', alignItems: 'center', gap: 12, fontSize: 14, color: ghRowColor }}>
        ...
        <span>{t('report.preferGithub')}</span>
        <a href="https://github.com/orchestra-mcp/orchestra/issues/new" target="_blank" rel="noopener" style={{ color: '#00e5ff', textDecoration: 'none', fontWeight: 500, marginInlineStart: 'auto' }}>
          {t('report.openIssue')} &rarr;
        </a>
      </div>
    </div>
  )
