  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, color: textPrimary, margin: 0, letterSpacing: '-0.02em' }}>{t('auth.magicLoginTitle')}</h1>
        <p style={{ fontSize: 14, color: textMuted, marginTop: 8 }}>{t('auth.magicLoginSubtitle')}</p>
      </div>
      ...
      <div className="auth-card" style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 20, padding: '32px 28px', backdropFilter: 'blur(16px)', boxShadow: cardShadow }}>
        {sent ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>{t('auth.magicCodeSent')}</div>
            <div style={{ fontSize: 13, color: hintColor, marginBottom: 24, lineHeight: 1.6 }}>
              {t('auth.magicCodeSentTo')}<br /><strong style={{ color: textPrimary }}>{email}</strong>
            </div>
            <button
          ...
          <>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              >
                {loading ? t('auth.sending') : t('auth.sendMagicCode')}
              </button>
            </form>

            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '24px 0' }}>
              <div style={{ flex: 1, height: 1, background: dividerBg }} />
              <span style={{ fontSize: 12, color: dividerText }}>{t('common.or')}</span>
              <div style={{ flex: 1, height: 1, background: dividerBg }} />
            </div>
            ...
            <Link href="/login" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px', borderRadius: 10, border: `1px solid ${cardBorder}`, textDecoration: 'none', fontSize: 13, fontWeight: 500, color: textPrimary }}>
              <i className="bx bx-lock-alt" style={{ fontSize: 15 }} /> {t('auth.signInWithPassword')}
            </Link>
          </>
        )}
      </div>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: footerText, marginTop: 24 }}>
        {t('auth.noAccount')}{' '}
        <Link href="/register" style={{ color: '#00e5ff', fontWeight: 600, textDecoration: 'none' }}>{t('common.createAccount')}</Link>
      </p>
    </div>
  )
}
