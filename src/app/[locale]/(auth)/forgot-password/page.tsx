  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, color: textPrimary, margin: 0, letterSpacing: '-0.02em' }}>{t('auth.resetYourPassword')}</h1>
        <p style={{ fontSize: 14, color: textMuted, marginTop: 8 }}>{t('auth.resetSubtitle')}</p>
      </div>
      ...
      <div className="auth-card" style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 20, padding: '32px 28px', backdropFilter: 'blur(16px)', boxShadow: cardShadow }}>
        {sent ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: textPrimary, marginBottom: 10 }}>{t('auth.checkYourEmail')}</div>
            <div style={{ fontSize: 13, color: hintColor, marginBottom: 24, lineHeight: 1.6 }}>
              {t('auth.codeSentTo')} <strong style={{ color: textPrimary }}>{email}</strong>.<br />{t('auth.codeExpires')}
            </div>
            <button
