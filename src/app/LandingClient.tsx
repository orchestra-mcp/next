...
export default function LandingClient({ data }: { data: LandingData }) {
  ...
  const stats: Array<{ value: string; label: string }> = homepage.stats?.map(s => ({ value: s.value, label: s.label })) ?? [
    { value: '290', label: 'MCP Tools' },
    { value: '36',  label: 'Plugins' },
      ...
      <main>
        ...
        <FeatureSection
          ...
          visual={<TerminalBlock lines={[
            ...
            { dim: true, text: `... ${parseInt(totalTools) - 8} more tools` },
          ]} />}
        />
        ...
        <FeatureSection
          ...
          title="In-process, not HTTP polling"
          description="4 core plugins ship inside the 17MB binary. Optional plugins install on demand. In-process routing means zero network overhead locally."
          bullets={['Single binary: orchestra serve', '4 core plugins bundled', '32 optional plugins on demand', 'Go + Rust + Swift + Kotlin + C# plugins']}
          accentColor="#a900ff"
          reverse
