...
async function getLandingData() {
  try {
    ...
    const [homepageRes, pricingRes, blogRes] = await Promise.all([
      fetch(`${apiBase}/api/settings/homepage`, { next: { revalidate: 60 } }),
      fetch(`${apiBase}/api/settings/pricing`, { next: { revalidate: 60 } }),
      fetch(`${apiBase}/api/settings/blog`, { next: { revalidate: 60 } }),
    ])
