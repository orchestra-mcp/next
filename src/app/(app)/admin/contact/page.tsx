...
export default function AdminContactPage() {
  ...
  const expandedBg = isDark ? 'rgba(0,229,255,0.03)' : 'rgba(0,229,255,0.02)'
  ...
  function StatusBadge({ status }: { status: AdminContact['status'] }) {
    const map = {
      ...
      read: { bg: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)', color: textMuted as string, border: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', label: 'Read' },
  ...
  return (
    <div style={{ padding: '28px 32px', background: pageBg, minHeight: '100vh' }}>
      ...
      <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 1.5fr 90px 110px 40px', padding: '11px 20px', borderBottom: `1px solid ${cardBorder}`, fontSize: 11, fontWeight: 600, color: textDim, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
          <div>Name</div>
          <div>Email</div>
          <div>Subject</div>
          <div>Status</div>
          ...
          return (
            <div key={msg.id}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{msg.email}</div>
                <div style={{ fontSize: 12, color: textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{msg.subject}</div>
                <div><StatusBadge status={msg.status} /></div>
                <div style={{ fontSize: 11.5, color: textDim }}>{fmt(msg.created_at)}</div>
              ...
              {isExpanded && (
                <div style={{
                  ...
                  borderTop: `1px solid ${isDark ? 'rgba(0,229,255,0.08)' : 'rgba(0,229,255,0.1)'}`,
                }}>
